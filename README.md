...
> Django ORM(Object Relation Mapping)을 활용하여 게시판 기능 구현하기
...
> 마이그레이션(migration)은 모델에 정의한 내용의 변경사항(데이터베이스(DB)의 스키마)을 관리한다.
>
...
* 특정 마이그레이션 파일이 데이터베이스에 반영될 때 실행되는 쿼리문은 다음고 ㅏ같다.
  ...
  -- Create model Article
  --
  CREATE TABLE "articles_article" ("id" integer NOT NULL PRIMARY KEY AUTOINCREMENT, "title" varchar(10) NOT NULL, "content" text NOT NULL, "created_at" datetime NOT NULL, "update_at" datetime NOT NULL);
  COMMIT;
...
* `shell_plus` 에서는 `django_extensions`를 설치 후 `INSTALLED_APPS`에 등록하고 활용해야한다.
...
# article = Article(title = '1번 글', content = '1번 내용')
article.save()
...
* 데이터베이스에 저장되면, `id`값이 자동으로 부여된다. `.save()`를 호출하기 전에는 `None`이다.
...
* 특정(단일) 데이터 조회
  ...
  * `.get()`는 그 결과가 여러개이거나 없는 경우 오류를 발생시킴.
  * 따라사, 단일 데이터 조회시 (`primary key`를 통해)에만 사용한다.
...
* 특정 데이터 조회
  ...
  * 리턴되는 값은 `QuerySet` 오브젝트
  * `.filter()`는 없는 경우/하나인 경우/여러개인 경우 모두 `QureySet` 리턴
...
```python
Article.objects.all()[0] # LIMIT : 1
Article.objects.all()[2] # LIMIT : 1 OFFSET : 2
Article.objects.all()[:3] # LIMIT : 3 OFFSET : 1~3
```
...
Article.objects.order_by('title') # title를 기준으로 내림차순 정렬
```
